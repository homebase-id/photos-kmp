...
**Batch G — Settings & Backup + green accent (done, review-clean, verified, Redmi-QA'd on `photos-ui-batch-g` `8d0bff5..aa7b11f`; 2026-07-26 night; = the MVP-finishing batch after the owner shelved F):**
- **Owner rulings this batch:** accent → **Conservatory moss green** restored, accent-tokens-only (light `5E7A52`/`D5E0C7`, dark `A6C394`/`3C4D30` + light Secondary-container pair; neutral GP surfaces untouched — `design-direction` memory holds). **Delete→bin still parked** ("we will talk later") — branch verified to not touch delete routing. **Storage meter OMITTED** — recon proved no quota/usage API exists anywhere in homebase-api (only per-file `fileByteCount`).
- **KEY RECON FINDING:** the plan doc's "heavy part" (iOS backup crawler) **already shipped in `32ebd7e`** — real `PHAssetCrawler` (nativeMain Kotlin over Photos framework), BGTask registration, working BackupView sheet. Only the JVM binding still uses `StubPhotoLibraryCrawler`. Batch G's real iOS gap was event-driven parity only.
- **Shared:** `settings/SettingsViewModel` (`SettingsUiState`: identity/displayName/initials; `refresh()` with already-loaded guard). **Narrow-seam ctor** (raw `StateFlow`s + `owner::load` ref) because `OwnerSessionRepository`/`PublicIdentityRepository` are final classes with no fakes. **Deliberately NO `logout()` intent** — the `_authState` flip destroys `viewModelScope` mid-call (warning comments in `YouAuthFlowManager.kt:397` / `DriveSyncManager.kt:444`); logout stays on Android `lifecycleScope` (MainActivity) / iOS detached `Task`. Wired via previously-unused `OwnerSessionRepository` (displayName + `initials()`). `:shared:jvmTest` 1206 green (+7 the extra from E's ledger).
- **Android:** `SettingsScreen` (account header, Backup/About/Sign out rows; version via `packageManager` — `BuildConfig` is disabled in gradle) + `BackupScreen` resurrected from the Batch-A-deleted `BackupStatusCard` (`git show 51b6ee9^:...`): permission launchers, enable-with-0-folders→picker rule, `FolderPickerSheet`, **`BackupScheduler.enable/disable(context)` beside every `vm.onToggle`** (the shared VM never schedules). Routes `settings`/`backup`; Timeline's logout dialog removed (account button → Settings). `SettingsFlowTest` (6) + `TimelineScreenTest`/`AppShellNavTest` updated.
- **iOS:** `SettingsView` **sheet** from the account button (`router.path` is Collections-tab-only — don't push from Timeline), `NavigationLink` → existing `BackupView` (now ids `backup-screen`/`backup-folders`; the Timeline cloud `backup-button` is REMOVED); `PhotoLibraryObserver` (`PHPhotoLibraryChangeObserver`, 5s debounce → `BGProcessingTask` at +60s; app-lifetime; fires only while app alive — MediaWatch parity, not instant upload); `Theme.swift` moss accent + verifier-caught `HomeTabView` `.tint` (tab chrome sat outside per-screen tints and stayed system blue, `2741286`). New files hand-registered in pbxproj (classic groups — NEVER `xcodegen generate`). `SettingsUITest` + `LogoutUITest` repointed.
- **Ids (both platforms):** `settings-root/account/backup/about/signout`, `backup-screen/toggle/now/folders`, keep `account-button`/`logout-confirm`; Android extras `backup-back`, `backup-progress`, picker tags; iOS `settings-done`. Asymmetry ledgered: iOS `backup-folders` renders only while enabled; Android's row is always present.
- **Verified:** full matrix green — jvmTest 1206/0, shared android+iosSim compile, assembleDebug + androidTest compile, iOS build-for-testing + XCUITests 12 pass/0 fail (SettingsUITest 3/3, LogoutUITest, even MainFlowUITest passed this run; 7 pre-existing data-state skips). One K/N link-cache flake (`--rerun-tasks` workaround held). Report: `2026-07-26-ui-batch-g-verification.md`.
- **On-device QA DONE (Redmi, 2026-07-26 night):** `install -r` kept login; Settings renders shared-state header (Peter Parker / initials in moss circle), Backup screen (toggle Off honest, Back-up-now disabled, folders row), sign-out dialog exact copy (cancelled, never confirmed), dark-moss accent live on the SDK-30 static fallback. iOS sim same via XCUITest + verifier live pass.
- **Review (whole branch): clean, merge-ready.** Fix wave `aa7b11f`: observer now also installs on first mid-session Photos grant (was launch-only — the enabling session got no change nudges); observer state main-queue-confined; Android folder sheet opens only after grant (was: empty sheet over the denial snackbar).
- **Deferred (ledgered):** `remember { resolver() }` ViewModels are never cleared → each Settings/Backup open leaks one Eagerly collector on singleton flows (**systemic since Batch A**, incl. BackupViewModel — cleanup batch candidate: `DisposableEffect`/`deinit` + `vm.clear()`); nested `NavigationStack` in pushed BackupView (works on iOS 26, flatten if it misbehaves); Android/iOS version strings disagree (0.1 vs 1.0); JVM crawler still stubbed (fine — no JVM app). **Env note:** on the Redmi (MIUI/A11), argent `describe` lags navigation and raw uiautomator can read a phantom window from another app — visual QA unaffected; instrumented tests unaffected (sibling of E's iOS-sim describe quirk).
- **Open owner calls:** Delete→bin routing; final iOS app icon; merge `photos-ui-batch-g` → main.
