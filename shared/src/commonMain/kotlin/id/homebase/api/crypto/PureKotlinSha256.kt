...
/**
 * Pure-Kotlin SHA-256 (FIPS PUB 180-4 §6.2).
 *
 * Used by [HashUtil.sha256Sync] for callers that must hash synchronously and
 * cannot suspend through `cryptography-kotlin`'s WebCrypto backend
 * (notably [id.homebase.api.common.OdinId]'s constructor, which is invoked
 * from non-suspending KSerializer code).
 *
 * Not exposed as part of the public API — call [HashUtil.sha256Sync] instead.
 * That way the implementation choice (pure-Kotlin vs. eventual WebCrypto-sync
 * polyfill) stays an internal detail.
 */
